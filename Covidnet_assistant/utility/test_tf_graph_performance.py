...
def parse_args():
    ...
    parser.add_argument(
        "--models_parent",
        action="store_true",
        help="test a bunch of models under model",
    )
    parser.add_argument(
    ...
    with tf.Session() as sess:
        ...
        if not latency_only:
            ...
            (
                ...
                specificity,
                sensitivity,
            ) = test_auc(sess, test_data_path, batch_size, binary_class)
            print("AUC:", auc)
...
if __name__ == "__main__":
    ...
    if args.models_parent:
        ...
        with open(
            f'test_result_{str(args.model).replace("/", "_")}.json',
            "w",
            encoding="utf-8",
        ) as f:
